...
@section('breadcrumbs')
    <x-admin.breadcrumbs :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Newsletter Subscribers']]" />
@endsection
...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('admin.newsletter-subscribers.index') }}" class="flex-1 flex gap-3">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Subscribed At</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $subscriber->email }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $subscriber->is_subscribed ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">
                                {{ $subscriber->is_subscribed ? 'Subscribed' : 'Unsubscribed' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $subscriber->created_at->format('M d, Y') }}</td>
                            ...
                            <div class="relative dropdown-container">
                                ...
                                <div class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 py-1 z-50">
                                    <form id="delete-form-{{ $subscriber->id }}"
                                          action="{{ route('admin.newsletter-subscribers.destroy', $subscriber) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button type="button"
                                                onclick="confirmAction('Delete Subscriber', 'Are you sure you want to delete this subscriber?', () => document.getElementById('delete-form-{{ $subscriber->id }}').submit())"
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500">No subscribers found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
