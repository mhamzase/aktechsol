...
@section('breadcrumbs')
    <x-admin.breadcrumbs :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Testimonials']]" />
@endsection

@section('content')
    <x-admin.page-banner title="Testimonials" subtitle="Manage client feedback" stats="{{ $testimonials->total() }} total testimonials" />
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('admin.testimonials.index') }}" class="flex-1 flex gap-3">
        ...
        <div class="overflow-x-auto overflow-y-visible">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            @if($testimonial->getPhotoUrl())
                                <img src="{{ $testimonial->getPhotoUrl() }}" class="h-10 w-10 rounded-full object-cover">
                            @else
                                <div class="h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-semibold text-xs">
                                    {{ strtoupper(substr($testimonial->client_name, 0, 1)) }}
                                </div>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $testimonial->client_name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $testimonial->company ?? '—' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $testimonial->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <div class="relative dropdown-container">
                                ...
                                <div class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 py-1 z-50">
                                    ...
                                    <form id="delete-form-{{ $testimonial->id }}"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="button"
                                                onclick="confirmAction('Delete Testimonial', 'Are you sure you want to delete this testimonial?', () => document.getElementById('delete-form-{{ $testimonial->id }}').submit())"
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">No testimonials found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
