...
@section('breadcrumbs')
    <x-admin.breadcrumbs :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Projects']]" />
@endsection

@section('content')
    <x-admin.page-banner title="Projects" subtitle="Manage your portfolio projects" stats="{{ $projects->total() }} total projects" />
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('admin.projects.index') }}" class="flex-1 flex gap-3">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            @if ($project->getFeaturedImageUrl())
                                <img src="{{ $project->getFeaturedImageUrl() }}" class="h-10 w-16 object-cover rounded">
                            @else
                                <div class="h-10 w-16 bg-gray-200 rounded flex items-center justify-center text-gray-400 text-xs">No img</div>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $project->title }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $project->client_name ?? '—' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $project->sort_order }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $project->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <div class="relative dropdown-container">
                                ...
                                <div class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 py-1 z-50">
                                    ...
                                    <form id="delete-form-{{ $project->id }}"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="button"
                                                onclick="confirmAction('Delete Project', 'Are you sure you want to delete \'{{ addslashes($project->title) }}\'?', () => document.getElementById('delete-form-{{ $project->id }}').submit())"
