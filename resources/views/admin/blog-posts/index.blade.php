...
@section('breadcrumbs')
    <x-admin.breadcrumbs :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Blog Posts']]" />
@endsection
...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('admin.blog-posts.index') }}" class="flex-1 flex gap-3">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            @if($post->getFeaturedImageUrl())
                                <img src="{{ $post->getFeaturedImageUrl() }}" class="h-10 w-16 object-cover rounded">
                            @else
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $post->published_at?->format('M d, Y') ?? '—' }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $post->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <div class="relative dropdown-container">
                                ...
                                <div class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 py-1 z-50">
                                    ...
                                    <form id="delete-form-{{ $post->id }}"
                                          action="{{ route('admin.blog-posts.destroy', $post) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button type="button"
                                                onclick="confirmAction('Delete Post', 'Are you sure you want to delete \'{{ addslashes($post->title) }}\'?', () => document.getElementById('delete-form-{{ $post->id }}').submit())"
