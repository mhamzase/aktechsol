@forelse($posts as $post)
    <a href="{{ route('blog.show', $post->slug) }}"
       class="block bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition">
        @if($post->getFeaturedImageUrl())
            <img src="{{ $post->getFeaturedImageUrl() }}" alt="{{ $post->title }}" class="h-48 w-full object-cover">
        @endif
        <div class="p-5">
            <p class="text-xs text-blue-600 font-semibold">{{ $post->category?->name ?? 'General' }}</p>
            <h3 class="text-xl font-semibold text-gray-900 mt-1">{{ $post->title }}</h3>
