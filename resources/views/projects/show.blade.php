...
<section class="py-20 bg-white">
    <div class="max-w-4xl mx-auto px-4">
        @if($project->getFeaturedImageUrl())
            <img src="{{ $project->getFeaturedImageUrl() }}" alt="{{ $project->title }}" class="w-full h-64 object-cover rounded-xl mb-8">
        @endif
        ...
        @if($project->project_url)
            <a href="{{ $project->project_url }}" target="_blank" class="inline-block mt-6 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">Visit Project</a>
        @endif
        @if(count($gallery))
            <div class="mt-8">
        ...
        @endif
        <a href="{{ route('projects.index') }}" class="inline-block mt-8 text-blue-600 hover:underline">&larr; Back to Projects</a>
    </div>
</section>
@endsection
